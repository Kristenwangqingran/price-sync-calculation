...
import (
	...
	"git.garena.com/shopee/seller-server/seller-listing/service-kits/ssk/component/logger"
)

var GenIDWorker *IdWorker

func NewGenIDWorker() {
	var err error
	var index int64
	if i := os.Getenv("INDEX"); i != "" {
		index, err = strconv.ParseInt(i, 10, 9)
		if err != nil {
			panic(err)
		}
	}

	GenIDWorker, err = NewIdWorker(index)
	if err != nil {
		panic(err)
	}
}

func GetGenIDWorker() *IdWorker {
	return GenIDWorker
}
...
func NewIdWorker(NodeId int64) (*IdWorker, error) {
	...
	if NodeId > maxNodeId || NodeId < 0 {
		logger.Errorf(context.TODO(), "NodeId Id can't be greater than %d or less than 0", maxNodeId)
		return nil, errors.New(fmt.Sprintf("NodeId Id: %d error", NodeId))
	}
	if districtId > maxDistrictId || districtId < 0 {
		logger.Errorf(context.TODO(), "District Id can't be greater than %d or less than 0", maxDistrictId)
	...
	logger.Infof(context.TODO(), "worker starting. timestamp left shift %d, District id bits %d, worker id bits %d, sequence bits %d, workerid %d", timestampLeftShift, DistrictIdBits, NodeIdBits, sequenceBits, NodeId)
	return idWorker, nil
}
...
func (id *IdWorker) NextId(ctx context.Context) (int64, error) {
	id.mutex.Lock()
	defer id.mutex.Unlock()
	return id.nextid(ctx)
}

// NextIds get snowflake ids.
func (id *IdWorker) NextIds(ctx context.Context, num int) ([]int64, error) {
	if num > maxNextIdsNum || num < 0 {
		logger.Errorf(ctx, "NextIds num can't be greater than %d or less than 0", maxNextIdsNum)
...
func (id *IdWorker) nextid(ctx context.Context) (int64, error) {
	...
	if timestamp < id.lastTimestamp {
		logger.Errorf(ctx, "clock is moving backwards.  Rejecting requests until %d.", id.lastTimestamp)
